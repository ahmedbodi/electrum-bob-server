...
The most up-to date version of this document is available at:
    ...
    https://github.com/ahmedbodi/electrum-bob-server/blob/master/HOWTO.md
...
document, we assume that user is called 'dobbscoin'). We also assume the
dobbscoin user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run dobbscoind and keep a copy of the full blockchain,
which is roughly 6 GB (February 2016). Ideally you have a machine with 4 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit dobbscoind 
to 8 concurrent connections by disabling incoming connections. electrum-bob-server may
...
CPU speed is less important than fast I/O speed. electrum-bob-server makes uses of one core 
only leaving spare cycles for dobbscoind. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running dobbscoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `dobbscoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser dobbscoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - dobbscoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/dobbscoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download dobbscoind

We currently recommend dobbscoind master

If you prefer to compile dobbscoind, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - dobbscoin
    $ cd ~/src && git clone https://github.com/dobbscoin/dobbscoin-source.git -b master
    $ cd dobbscoin
    $ ./autogen.sh
    $ ./configure --disable-wallet --without-miniupnpc
    $ make
    $ strip src/dobbscoind src/dobbscoin-cli src/dobbscoin-tx
    $ cp -a src/dobbscoind src/dobbscoin-cli src/dobbscoin-tx ~/bin

### Step 3. Configure and start dobbscoind

In order to allow Electrum to "talk" to `dobbscoind`, we need to set up an RPC
username and password for `dobbscoind`. We will then start `dobbscoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.dobbscoin
    $ $EDITOR ~/.dobbscoin/dobbscoin.conf

Write this in `dobbscoin.conf`:
...
If you have an existing installation of dobbscoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ dobbscoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `dobbscoind`:

    $ dobbscoind

Allow some time to pass, so `dobbscoind` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ dobbscoin-cli getinfo

Before starting the Electrum server your dobbscoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start dobbscoind at boot
time, running as the 'dobbscoin' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/ahmedbodi/electrum-bob-server.git
    $ cd electrum-bob-server
    $ sudo configure
...
### Step 7. Import blockchain into the database
...
Electrum reads a config file (/etc/electrum-bob.conf) when starting up. This
file includes the database setup, dobbscoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-bob.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-bob-server.
If you're using the user `dobbscoin` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "dobbscoin hard nofile 65536" >> /etc/security/limits.conf
     echo "dobbscoin soft nofile 65536" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u dobbscoin -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close dobbscoind for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting dobbscoind (together with electrum-bob-server) on a weekly basis to clear out unconfirmed
...
The magic moment has come: you can now start your Electrum server as root (it will su to your unprivileged user):
    ...
    # electrum-bob-server start

Note: If you want to run the server without installing it on your system, just run 'run_electrum_bob_server" as the
unprivileged user.
...
If you want to stop Electrum server, use the 'stop' command:
    ...
    # electrum-bob-server stop


If your system supports it, you may add electrum-bob-server to the /etc/init.d directory.
...
safely whenever your machine is rebooted.
    ...
    # ln -s `which electrum-bob-server` /etc/init.d/electrum-bob-server
    # update-rc.d electrum-bob-server defaults
...
dobbscoins to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-bob and we'll try to congratulate you
on supporting the community by running an Electrum-BOB node.
